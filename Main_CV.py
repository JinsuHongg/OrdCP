...
for wt in decay_val:
    ...
    for i in range(0, 4):
        ...
        loss_fn = cross_entropy # NLLLoss(weight=torch.tensor([imbalance_ratio, 1-imbalance_ratio])).to(device)
        ...
        for t in range(num_epoch):
            ...
            print(f'Epoch {t+1}: Lr: {actual_lr:.5f}, Train loss: {train_loss:.3f}, Test loss: {test_loss:.3f}, HSS: {HSS_score:.3f}, TSS: {TSS_score:.4f}, F1: {F1_score:.4f}, Duration(min):  {duration:.2f}')
