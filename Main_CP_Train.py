...
# function for find optimal weight decay from cross-validation result
def find_weight(model:str = 'Mobilenet', data:str = 'HMI-CTnuum'):
    file_dir = f'./Results/CV/{model}*{data}*'
    files = glob.glob(file_dir)
    
    # find all the combinations of learning rates and weight decays
    result = []
    for file in files:
        df_partial = pd.read_csv(file)
        df_partial['CSS'] = (df_partial['HSS']*df_partial['TSS'])**0.5
        idx = df_partial.groupby(['Train_p', 'Test_p'])['CSS'].transform('max') == df_partial['CSS']
        data_series = df_partial.loc[idx, ['weight decay', 'CSS']].mean()
        result.append([float(data_series['weight decay']), float(data_series['CSS'])])
    
    # find optimal weight decay (which make the maximum CSS score)
    df = pd.DataFrame(result, columns = ['weight decay', 'CSS'])
    index = int(df['CSS'].argmax())
    final = df.iloc[index, :]
    
    return float(final['weight decay'])

# CUDA for PyTorch
torch.cuda.empty_cache()
use_cuda = torch.cuda.is_available()
device = torch.device('cuda:1') #'cuda' if use_cuda else 'cpu'
...
lr = opt.lr
max_lr = opt.max_lr
wt = find_weight(model = model_name, data = channel_tag)

print(f'Hyper parameters: batch_size: {batch_size}, number of epoch: {num_epoch},' + \
      f'learning rate: {lr}, max learning rate: {max_lr}, decay value: {wt}')


# For calibration, we use partition 1 and 2 as training, partition 3: calibration, and partition 4: testing.
# Define dataset here! 
train = [1, 2]
test = 4
train_list = [f'24image_multi_GOES_classification_Partition{train[0]}.csv', 
            f'24image_multi_GOES_classification_Partition{train[1]}.csv']

test_file = f'24image_multi_GOES_classification_Partition{test}.csv'
...
for t in range(num_epoch):
    ...
    training_result.append([t, train, test, actual_lr, wt, train_loss, test_loss, HSS_score, TSS_score, F1_score, duration])
    torch.cuda.empty_cache()
    ...
    if train_loss != train_loss:
        print('Loss Error!')
        print(f'{model_name}, {channel_tag}, train {train}, test {test}')
        error_result = save_path + "error/" + f"{model_name}_{channel_tag}_ErrLog_{date.year}{date.month:02d}_train{train[0]}{train[1]}_test{test}.npy"
    ...
    if best_hsstss < check_hsstss:
        ...
        PATH = save_path + f"regmodel/{model_name}_{channel_tag}_freeze{opt.freeze}_{date.year}{date.month:02d}_train{train[0]}{train[1]}_test{test}.pth"
    # save model
        torch.save({
                'epoch': t,
        ...
        log_path = save_path + 'log/' + f'{model_name}_{channel_tag}_freeze{opt.freeze}_{date.year}{date.month:02d}_train{train[0]}{train[1]}_test{test}_' + \
...
total_save_path = save_path + f'CV/{model_name}_{channel_tag}_freeze{opt.freeze}_{date.year}{date.month:02d}_result_CPtrain_lr{-math.log10(lr):.1f}_wt'
if wt != 0:
